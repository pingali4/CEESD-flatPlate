...
if [ -z ${CONDA_PATH+x} ]; then
  ...
  ./install.sh --env-name=mirgeDriver.Y1flatPlate --git-ssh --branch=y1-production
else
  echo "Using existing Conda installation, ${CONDA_PATH}"
  ./install.sh --conda-prefix=$CONDA_PATH --env-name=mirgeDriver.Y1flatPlate --git-ssh --branch=y1-production
fi
